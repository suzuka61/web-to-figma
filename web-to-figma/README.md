...
A Chrome extension that captures any web page or element and pastes it directly into Figma as editable layers.
...
- **CORS-free by design** — `declarativeNetRequest` injects CORS headers and strips CSP, no need for separate CORS extensions
...
| `declarativeNetRequest` | Inject CORS headers and remove CSP restrictions |
| `<all_urls>` | Apply CORS/CSP rules to any site, proxy cross-origin assets |
...
一个 Chrome 扩展，一键抓取任意网页或元素，直接粘贴到 Figma 中作为可编辑图层。
...
- **零 CORS 配置** — `declarativeNetRequest` 自动注入 CORS 头并移除 CSP，无需安装额外插件
...
| `scripting` | 注入采集脚本到页面 |
| `storage` | 存储扩展偏好设置 |
| `declarativeNetRequest` | 注入 CORS 头并移除 CSP 限制 |
| `<all_urls>` | 对任意网站生效 CORS/CSP 规则，代理跨域资源 |
